...
class EmployerCompany(models.Model):
    background_picture = models.ImageField(upload_to='company_background_pictures/', blank=True, verbose_name=_('Фон'))
...
class Invitation(models.Model):
    ...
    user = models.ForeignKey('accounts.User', on_delete=models.CASCADE, related_name="invitations", verbose_name=_('Пользователь'))
    vacancy = models.ForeignKey(Vacancy, on_delete=models.CASCADE, verbose_name=_('Вакансия'))
...
class Tariff(models.Model):
    ...
    # Варианты тарифов
    TARIF_CHOICES = [
        (10000, _('Десять тысяч')),
        (20000, _('Двадцать тысяч')),
        (35000, _('Тридцать пять тысяч')),
    ]

    amount_choice = models.IntegerField(_('Вариант тарифа'), choices=TARIF_CHOICES, default=10000)
