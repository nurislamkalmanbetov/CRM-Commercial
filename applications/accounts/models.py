...
class Profile(models.Model):
    ...
    LANGUAGE_LEVEL_CHOICES = (
        ('Понимаю и разговариваю без проблем', 'Понимаю и разговариваю без проблем'),
        ('Если что-то не понимаю, то переспрашиваю', 'Если что-то не понимаю, то переспрашиваю'),
        ('Понимаю многое, но плохо говорю', 'Понимаю многое, но плохо говорю'),
        ('Понимаю немного, когда говорят очень медленно, но плохо говорю', 'Понимаю немного, когда говорят очень медленно, но плохо говорю'),
        ('Не разговариваю совсем', 'Не разговариваю совсем'),
    )
    ...
    gender = models.CharField('Пол', choices=GENDER_CHOICES, max_length=1, default='', blank=True)
    ...
    # languages info
    german = models.CharField('Знание немецкого языка', choices=LANGUAGE_LEVEL_CHOICES, max_length=150, default='', blank=True)
    english = models.CharField('Знание английского языка', choices=LANGUAGE_LEVEL_CHOICES, max_length=150, default='', blank=True)
    turkish = models.CharField('Знание турецкого языка', choices=LANGUAGE_LEVEL_CHOICES, max_length=150, default='', blank=True)
    russian = models.CharField('Знание русского языка', choices=LANGUAGE_LEVEL_CHOICES, max_length=150, default='', blank=True)
    chinese = models.CharField('Знание китайского языка', choices=LANGUAGE_LEVEL_CHOICES, max_length=150, default='', blank=True)
...
class ProfileInPayment(Profile):
    objects = ProfileInRegistrationManager()
...
# @receiver(post_save, sender=LogEntry)
# def change_agreement_download_date(sender, instance, created, **kwargs):

#     if created:
#         if instance.action_flag == 2:
#             msg = json.loads(instance.change_message)
#             profile = Profile.objects.filter(id=instance.object_id).first()
...
#                     for i in ['agreement1', 'agreement2', 'agreement3',
#                               'act1', 'act2', 'act3', 'closure1', 'closure2', 'closure3', ]:

#                         if i in fields:

#                             if i in ['closure1', 'closure2', 'closure3']:
#                                 profile.is_refused = True
#                                 if 'is_refused' not in update_fields:
#                                     update_fields.append('is_refused')
#                                 continue

#                             setattr(profile, f'{i}_download_date', datetime.date.today())
#                             update_fields.append(f'{i}_download_date')
...
#                 if getattr(profile, i):
#                    setattr(profile, f'{i}_download_date', datetime.date.today())
#                    update_fields.append(f'{i}_download_date')
#             profile.save(update_fields=update_fields)
            
# @receiver(post_save, sender=LogEntry)
# def change_immat_download_date(sender, instance, created, **kwargs):
...
#                     if 'immatrikulatuon' in fields:
#                         setattr(profile, 'immatrikulation_download_date', datetime.date.today())
#                         profile.save(update_fields=['immatrikulation_download_date'])
...
# @receiver(post_save, sender=LogEntry)
# def change_confirmation_of_filefields(sender, instance, created, **kwargs):
#
#     if created:
#         if instance.action_flag == 2 and instance.object_repr in ['Вакансия', 'Отказавшиеся', 'Собеседование',
#                                                                   'Термин', 'Справочная информация', 'На регистрации',
...
#
#                     for field in fields:
#                         if field in ['study_certificate', 'photo_for_schengen', 'zagranpassport_copy',
#                                      'passport_copy', 'fluorography_express', ]:
#                             setattr(profile, f'{field}_confirm', True)
#                             update_fields.append(f'{field}_confirm')
#                     profile.save(update_fields=update_fields)
